...
def output(pdb, vox, S):
    fnam = os.path.basename(pdb).split('.')[0] + '.h5'
    if os.path.exists(fnam):
        os.remove(fnam)
    
    f = h5py.File(fnam, 'w')
    f['density'] = S
    f['vox'] = np.array([vox, vox, vox])
    f.close()
        
def parse_cmdline_args(script_name, ):
    """
    """
    description = "Calculate the number of electrons per cubic angstrom from the molecule decribed in a pdb file."
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('pdb_id', type=str, \
                        help="protein database ID, e.g. 1sx4")
    parser.add_argument('--voxel_size', nargs='?', const=0.5, type=float, \
                        help="side length of voxel cube in angstroms")
    parser.add_argument('--B_offset', nargs='?', const=30, type=float, \
...
if __name__ == '__main__':
    #pdb = sys.argv[1]
    #vox = float(sys.argv[2])
    args = parse_cmdline_args('density_calculator_intelHD.py')
    
    S = render_molecule_from_pdb(args.pdb_id, args.voxel_size, B_offset=args.B_offset)
    
    output(args.pdb_id, args.voxel_size, S)
